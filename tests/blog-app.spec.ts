...
import { resetDatabase, createUser, loginUser, createBlog, baseUrl } from "./helpers"
...
test.describe("Blog Application", () => {
  ...
  test.describe("Navigation", () => {
    ...
    test("user can navigate to blogs page", async ({ page }) => {
      ...
      await expect(page).toHaveURL(`${baseUrl}/blogs`)
    })
    ...
    test("logged in user can navigate to create blog page", async ({
      ...
      await page.goto(`${baseUrl}/blogs/new`)

      await expect(page).toHaveURL(`${baseUrl}/blogs/new`)
    })
